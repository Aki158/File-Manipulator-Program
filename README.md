...
一般的なPC作業と言えば、マウスを使用してファイルをコピーしたり、開いたりするような操作をイメージするかもしれません。

より効率的に作業をしたい人は、キーボードを使用してショートカットキーを多用するかもしれません。
...
ターミナルでの操作は、キーボードのみで完結するため、マウスを使用する場合に比べて高速で、より多くの作業を迅速に実行することができます。

このスクリプトでは、ターミナルのコマンド入力を利用してファイルを操作することができます。

お目当てのコマンドが見つかれば、ファイル作業を効率化できるかもしれません!

💬「今のPC作業の効率化に使えるコマンドはないかな?」という目線で見ると楽しめます。
...
| エラーハンドリング | コマンドが正しく入力されていない場合は、`Command not found...`というメッセージが表示されて終了します。<br>コマンドの入力が、認識されなかったため、ファイルは生成されません。 |
